...
INSERT INTO driver_order(order_id,driver_id,pickup_time,distance,duration,cancellation) 
 VALUES(1,1,STR_TO_DATE("01-01-2021 18:15:34", "%m-%d-%Y %H:%i:%s"),'20km','32 minutes',''),
...
(7,2,STR_TO_DATE("01-08-2020 21:30:45", "%m-%d-%Y %H:%i:%s"),'25km','25mins',null),
(8,2,STR_TO_DATE("01-10-2020 00:15:02", "%m-%d-%Y %H:%i:%s"),'23.4 km','15 minute',null),
(9,2,null,null,null,'Customer Cancellation'),
(10,1,STR_TO_DATE("01-11-2020 18:50:20", "%m-%d-%Y %H:%i:%s"),'10km','10minutes',null);
...
-- How many rolls are ordered?
select count(roll_id) from customer_orders;

-- How many unique customers order were made?
select count(distinct customer_id) from customer_orders;

-- How many successful order delivered by each of the driver?
...
-- How many each type of roll is delivered?
...
-- How many veg and non veg customer ordered ?
select a.*, b.roll_name 
from
(select customer_id, roll_id, count(roll_id) num
...
group by customer_id, roll_id)a inner join rolls b on a.roll_id = b.roll_id;
 ...
 select * from
 (select *, rank() over(order by num desc) rnkk from
 (select order_id, count(roll_id) num from
 (select * from customer_orders
 where order_id in 
 (select order_id from
 (select *, case when cancellation in ('cancellation','customer cancellation') then 'c' else 'nc' end as order_cancel_details from driver_order) as a
...
group by order_id)c
 ...
 -- For each customr, how many delivered rolls had at least 1 change and how many had no changes?
...
select
hr_bucket,count(hr_bucket) from
(select *,
concat( cast(datepart(hour,order_date) as varchar),'-',cast( datepart(hour,order_date)+1 as varchar)) hr_bucket from customer_orders)a
group by hr_bucket;
...
--  What was the average time in minutes it took for each driver to arrives at the fasoos HQ to pickup the order?
...
b.driver_id,b.pickup_time,b.duration,b.cancellation,datediff(minute,a.order_date,b.pickup_time) diff
 from customer_orders a inner join driver_order b on a.order_id
where b.pickup_up_time is not null)a)b 
where rnk = 1)c
group by driver_id;
...
driver_order where duration is not null
)a;

-- What was the average speed for each driver for each delivery and do you notice any trend for these values?
select a.order_id,driver_id,cast(trim(replace(lower(distance),'km','')) as decimal(4,2)) distance,
